...
BEGIN
    ...
    FOR single_set IN set_cursor LOOP
        ...
        INSERT INTO TEMP_CANDIDATES (SELECT ITEMID FROM ITEMS
                                     WHERE ITEMID IN (SELECT ITEMID FROM TRANS
                                                      GROUP BY ITEMID
                                                      HAVING COUNT(*) >= support * no_of_trans / 100) MINUS (SELECT * FROM TEMP));
